...
exports.apikey = async (req, res, next) => {
  try {
    ...
    const uuidAPIKey = new UUIDAPIKey();
    const ApiFinalKey = uuidAPIKey.create();
    res.status(200).json({
      status: true,
      message : 'API Key and UUID',
      data : ApiFinalKey
    });
...
// Checking if API And UID (Secret Key) are correctly matching
exports.apikeycheck = async (req, res, next) => {
  try {
    ...
    const uuidAPIKey = new UUIDAPIKey();
    const ApiFinalKey = uuidAPIKey.check(apiKey,uuid);
    res.status(200).json({
      status: true,
      message : 'API Key and UUID Checking',
      data : ApiFinalKey
    });
...
// API Headder Verification
// function getApi(apiKey, uuid) {
